...
public interface TCmsContentMapper extends Mapper<TCmsContent> {
    ...
    List<TCmsContent> selectByConditionSousuo(TCmsContent content);
	...
	@Select("select news.content_id contentId, t_cms_content.title,t_cms_content.url,t_cms_content.user_id,t_cms_content.inputdate,t_cms_content.view_num viewNum,news.content description from t_cms_content,t_cms_content_news news where title like CONCAT('%',#{0},'%') and news.content_id=t_cms_content.content_id")
    List<Map<String,Object>> FuzzyQueryTitle(String title);

	@Select("select news.content_id contentId, t_cms_content.title,t_cms_content.author,t_cms_content.user_id,t_cms_content.inputdate,t_cms_content.view_num viewNum,news.content description from t_cms_content,t_cms_content_news news where  news.content_id=t_cms_content.content_id and news.content_id=#{0}")
	TCmsContent selectOneByContentId(String contentId);
